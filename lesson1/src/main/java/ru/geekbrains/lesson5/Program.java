            ...
            if (scanner.hasNextInt()) {
                ...
                try {
                    switch (no) {
                        ...
                        case 8 -> {
                            System.out.print("Укажите номер модели, которую нужно удалить: ");
                            if (scanner.hasNextInt()) {
                                int modelNo = scanner.nextInt();
                                scanner.nextLine();
                                editor3D.removeModel(modelNo);
                                System.out.printf("Модель %d была успешно удалена", modelNo);
                            } else {
                                System.out.println("Номер модели указан некорректно.");
                            }
                        }
                        case 9 -> {
                            System.out.print("Укажите номер текстуры, которую нужно удалить: ");
                            if (scanner.hasNextInt()) {
                                int modelNo = scanner.nextInt();
                                scanner.nextLine();
                                editor3D.removeTexture(modelNo);
                                System.out.printf("Текстура %d была успешно удалена", modelNo);
                            } else {
...
class EditorBusinessLogicalLayer implements BusinessLogicalLayer {
    ...
    @Override
    public void removeModel3D(int id) {
...
class EditorDatabase implements Database {
     ...
     * Предусловие. Выполняем проверку, что коллекция проинициализирована
     *
     * @param entity
     */
    @Override
    public void removeEntity(Entity entity) {
        ...
        //ПРЕДУСЛОВИЕ
        if (entities == null) {
            throw new RuntimeException("В коллекции нет сущностей.");
        }
